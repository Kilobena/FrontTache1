...
const Crash = ({ limit = null, hideFooter = false, hideExtras = false, horizontalOnMobile = false }) => {
  ...
  useEffect(() => {
    const loadGames = async () => {
      try {
        setLoading(true);
        const fetchedGames = await fetchGames(offset, 30, { category: "Gamzix" });
        if (offset === 0) {
  ...
  .filter((game) => 
    providerFilter === "all" || game.provider === providerFilter
  )
  .sort((a, b) => {
...
const displayedGames = limit ? filteredGames.slice(0, limit) : filteredGames;
  ...
  <div className="flex flex-wrap items-center justify-between mb-6 gap-y-4 sm:gap-y-0">
    {/* Left Section: Return Button and Heading */}
  ...
  <h2 className="text-lg font-bold">Featured Games</h2>
    ...
    <select className="bg-white text-black w-full px-4 py-2 rounded border border-gray-400 shadow-sm focus:ring-2 focus:ring-yellow-500 appearance-none">
      ...
      <select className="bg-white text-black w-full sm:w-auto px-4 py-2 rounded border border-gray-400 shadow-sm focus:ring-2 focus:ring-yellow-500 appearance-none">
