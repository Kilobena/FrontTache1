            ...
            <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1 sm:gap-0  lg:w-full">
                    {dateOptions.map((option, index) => (
                        <label
                            key={option.value}
                            className={`flex md:flex-1 rounded-lg sm:rounded-none lg:w-full lg:min-w-[11rem] lg:last:max-w-[20rem] lg:max-w-[13.5rem] md:gap-3 items-center ml-0.5 p-2.5 cursor-pointer first:rounded-tl-lg first:rounded-bl-lg last:rounded-tr-lg last:rounded-br-lg  ${selectedDate === option.value ? "bg-yellow-400 text-black shadow-md" : "bg-[#e2e2e2] text-gray-700"
                                } 
                    ...
                    {isDatePickerClose &&
                        <>
                            ...
                            <div className="flex justify-center mt-4 space-x-4">
                                <button className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-2 px-11  rounded-lg" disabled={!selectedDate} onClick={handleCloseDate} >CLOSE</button>
