    ...
    <>
      <header className="casino-header bg-[#383838] text-white md:mb-4 md:py-2 md:px-4 shadow-md">
        ...
        <div className="flex items-center">
          {/* Navigation Items */}
          <nav className="flex w-full items-center md:gap-5 md:px-4 whitespace-nowrap scrollbar-hide overflow-x-auto mr-auto">
            {GAMES_CATEGORY_NAV.map((item, index) => (
              <NavLink
                key={index}
                to={item.path}
                className={({ isActive }) =>
                  `flex flex-col min-w-[24%] md:min-w-max items-center justify-center gap-1 md:gap-2 md:px-2 py-2 md:rounded-lg text-xs md:text-sm font-semibold transition-all ${
              ...
              >
                ...
                <span className="text-center lg:text-[15px] text-[10px]">{item.label}</span>
              </NavLink>
            ))}
          </nav>
